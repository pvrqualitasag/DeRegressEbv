...
In einem zweiten Schritt werden die Gleichungen nach $Z_i'Z_i$ und $Z_{PA}'Z_{PA}$ aufgelöst. Eine direkte Lösung erhält man durch Division der beiden Gleichungen durcheinander. Setzen wir $\delta = (0.5 - r_{PA}^2)/(1 - r_i^2)$ und vereinfachen die Gleichungen, so erhalten wir 

$$ Z_i'Z_i = \delta Z_{PA}'Z_{PA} + 2\lambda(2\delta - 1)
$$

Setzen wir diese Beziehung in der zweiten der oberen Gleichungen ein und definieren $\alpha = 1/(0.5 - r_{PA}^2)$ erhalten wir folgende quadratische Gleichung für $Z_{PA}'Z_{PA}$:
